...
class TaskRepository extends \TYPO3\CMS\Extbase\Persistence\Repository
{
     ...
     */
    public function findSearchForm($search, $offset, $limit, $orderProperty, $orderStr, \Ast\Projectmanager\Domain\Model\Project $project = null, \Ast\Projectmanager\Domain\Model\User $user = null)
    {
        ...
        if (isset($project)) {
            $query->matching($query->logicalAnd($query->like('name', '%' . $search . '%'), $query->equals('project', $project)));
        } else {
			if (isset($user)) {
				$query->matching($query->logicalAnd($query->like('name', '%' . $search . '%'), $query->equals('taskMaster', $user)));
			} else {			
	 ...
	 * @param \Ast\Projectmanager\Domain\Model\User $user 
     */
    public function countSearchForm($search, \Ast\Projectmanager\Domain\Model\Project $project = null, \Ast\Projectmanager\Domain\Model\User $user = null)
    {
        $query = $this->createQuery();
        if (isset($project)) {
            $query->matching($query->logicalAnd($query->like('name', '%' . $search . '%'), $query->equals('project', $project)));
